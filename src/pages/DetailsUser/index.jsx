...
export default function DetailsUser() {
  ...
  }, [dataAfterParse?.Status, rejectStt]);
  ...
  useEffect(() => {
    ...
    setStatus(
      rejectStt === 9 && dataAfterParse?.Status === 'Ringing'
        ? 'Người gọi tắt máy'
            ...
            <form>
                    ...
                    customer?.customer?.levels.map((lv, index) => {
                      if (lv?.name) {
                        return (
                          <option
                            selected={
                              customer?.customer.dataUsers.levelId === lv.id
                            }
                            key={index}
                            value={lv.id}
                          >
                ...
                <select ref={cnIdRef}>
                  {channels &&
                    channels.map((cn, index) => (
                      <option
                        selected={
                          customer?.customer.dataUsers.channelId === cn.id
                        }
                        key={index}
                        value={cn.id}
                      >
                    ...
                    source.map((sc, index) => {
                      if (sc?.name) {
                        return (
                          <option
                            selected={
                              customer?.customer.dataUsers.sourceDataId ===
                              sc.id
                            }
                            value={sc.id}
                            key={index}
              ...
              <div className={styles.field}>
                ...
                <select ref={genderRef}>
                  <option
                    selected={customer?.customer.dataUsers.gender === 1}
                    value='1'
                  >
                    Nam
                  </option>
